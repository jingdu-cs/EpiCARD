"""COVID-19 dataset loader for HierEpiGNN.

Loads case-level genomic data, genetic similarity (chunked),
and county-level cumulative confirmed counts (wide format),
...
class COVIDDataset(EpidemicDataset):
    """COVID-19 epidemic dataset.

    Data sources:
    - case_data.csv: individual sequenced samples with lineage/location
    - covid_confirmed.csv: cumulative county-level confirmed counts (wide format)
    """
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self._unallocated_locs: set[tuple[str, str]] = set()
        super().__init__(cfg, dataset_name="covid")
    ...
    def _load_raw_data(self) -> None:
        """Load and preprocess all COVID-19 data sources."""
        covid_cfg = self.dataset_cfg

        # 1. Load case_data.csv
        self._load_case_data(covid_cfg["case_data"])

        # 2. Load covid_confirmed.csv (wide -> long format incidence)
        self._load_incidence(covid_cfg["incidence"])

        # 4. Build location_index. By default, take the union of case-file and
        # incidence-file (state, county) pairs (legacy behavior). When
        # ``restrict_to_incidence_locations`` is set, restrict to incidence
        # pairs only; this drops case-only locations (which carry no
        # regression supervision) from the forecaster, and downstream the
        # cases falling on those locations are removed from the case stream.
        restrict_to_incidence = bool(
            covid_cfg.get("restrict_to_incidence_locations", False)
        )
        all_locs: set[tuple[str, str]] = set()

        if not restrict_to_incidence:
        ...
        logger.info(
            "COVID location_index built: %d locations (restrict_to_incidence=%s)",
            len(self.location_index), restrict_to_incidence,
        )

        # 4b. When restricting, drop case rows whose loc_key fell outside the
        # restricted index. These case-only rows would otherwise become
        # dangling references (case_batch pointing to an invalid location).
        if restrict_to_incidence:
            n_before = len(self.cases_df)
            in_index = self.cases_df["loc_key"].apply(
                lambda lk: isinstance(lk, tuple) and lk in self.location_index
            )
            n_dropped = int((~in_index).sum())
            self.cases_df = self.cases_df[in_index].copy()
            logger.info(
                "COVID restrict_to_incidence_locations: dropped %d case rows "
                "(of %d) on case-only locations; kept %d rows on %d locations.",
                n_dropped, n_before, len(self.cases_df), len(self.location_index),
            )
        ...
        logger.info(
            "COVID date ranges: case_data=[%s, %s], incidence=[%s, %s]",
        ...
        logger.info(
            "COVID location overlap: case=%d, incidence=%d, overlap=%d",
    ...
    def _get_y_hist(self, window_end: datetime, K: int) -> Tensor:
        """Recent K observed daily/weekly counts per location.
    ...
    def _get_mask(
        ...
        1.0 for locations with cumulative count > 0 at window_end
        AND future data exists for at least the shortest horizon.
        0.0 for locations with FIPS=0 (statewide unallocated) or
        lacking future data.
        ...
        for loc_key, loc_idx in self.location_index.items():
            if loc_key in self._unallocated_locs:
                continue
            # Check for "unallocated" in county name
            if isinstance(loc_key, tuple) and len(loc_key) == 2:
                county = loc_key[1] if loc_key[1] else ""
                if "unallocated" in county.lower():
                    continue
    ...
    def _load_case_data(self, path: str) -> None:
        ...
        logger.info("Loading COVID case_data from %s", path)
        df = pd.read_csv(path)
        logger.info("COVID case_data loaded: %d rows", len(df))
        ...
        if "Subcategory" not in df.columns:
            df["Subcategory"] = "Nan"
        if "Strain" not in df.columns and "Strain" not in df.columns:
        ...
        self.cases_df = df
        logger.info(
            "COVID case_data processed: %d rows, %d unique locations",
            len(df),
            df["loc_key"].nunique(),
        )

    def _load_incidence(self, path: str) -> None:
        """Load covid_confirmed.csv (wide format), convert cumulative to incidence.

        The wide format has columns:
        countyFIPS, County Name, State, StateFIPS, <date_1>, <date_2>, ...
        ...
        """
        logger.info("Loading COVID confirmed incidence from %s", path)
        df = pd.read_csv(path)
        logger.info("COVID confirmed loaded: %d rows", len(df))
        ...
        for _, row in df.iterrows():
            fips = int(row.get("countyFIPS", 0)) if pd.notna(row.get("countyFIPS")) else 0
            county_name = str(row.get("County Name", ""))
            state_abbrev = str(row.get("State", ""))

            ns, nc = self.location_normalizer.from_fips(fips, county_name, state_abbrev)
            loc_key = (ns, nc)

            if fips == 0:
                # Track all unallocated FIPS=0 locations
                self._unallocated_locs.add(loc_key)
        ...
        # Aggregate duplicate (loc_key, date) pairs that arise
        # from multiple FIPS mapping to same normalized location
        if not self.incidence.empty:
            self.incidence = (
                self.incidence
        ...
        logger.info(
            "COVID incidence processed: %d %s records, %d unique locations",
            len(self.incidence),
            time_unit,
